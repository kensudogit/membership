...
1. **`RAILWAY_PUBLIC_DEPLOY.md`** - 完全公開モードデプロイガイド（詳細手順）
2. **`RAILWAY_DEPLOY_ACTION.md`** - 実行手順書
3. **`RAILWAY_DEPLOY_START.md`** - クイックスタートガイド
4. **`railway-env-template.txt`** - 環境変数設定テンプレート
5. **`RAILWAY_DEPLOY_CHECKLIST.md`** - デプロイチェックリスト
6. **`railway-deploy-execute.bat`** - デプロイ実行スクリプト
...
各サービスの公開URLを確認し、フロントエンドの環境変数に反映してください。
...
1. **eureka-server**
2. **config-server**
3. **membership-service, payment-service, lesson-service, store-service, integration-service**
4. **api-gateway**
5. **frontend**
...
**注意**: 実際のURLは、Railwayが自動生成するため、各サービスの「Settings」タブで実際の公開URLを確認してください。
...
- **`RAILWAY_PUBLIC_DEPLOY.md`** - 完全公開モードデプロイガイド（詳細手順）
- **`RAILWAY_DEPLOY_ACTION.md`** - 実行手順書
